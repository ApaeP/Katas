...
class Sudoku
  ...
  def solve
    tile_to_solve = 0
    while @puzzle.include?(0)
      raise StandardError.new "Invalid grid" if tile_to_solve <= -1
      solve_tile(@empty_tiles[tile_to_solve]) ? tile_to_solve += 1 : tile_to_solve -= 1
    end
    grid_is_valid?
  end
  ...
  def create_hash
    hash, squ_i, row_i = {}, 0, 0
    @puzzle.each_with_index do |number, i|
      hash[i] = { val: number, row: ((i+1).fdiv(9).ceil) - 1, col: i % 9, editable: number == 0 }
      case i
      when 0..26
        case row_i
        when 0..2; squ_i = 0
...
# codewars_id: 5296bc77afba8baa690002d7   |   14/10/2020 - 17:32:23
